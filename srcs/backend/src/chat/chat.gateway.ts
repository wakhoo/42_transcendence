    ...
    async handleConnection(client: Socket) {
        try {
            ...
            */
            ...
            console.log(`[MODE DEV] Faux utilisateur ${payload.sub} connecté (socket ${client.id})`);
        } catch (error) {
            console.error("Erreur dans handleConnection :", error);
            // client.disconnect(); // On l'empêche de nous jeter au cas où !
        }
    }
